...
local redzlib = {
	Themes = {
		...
		CyberBlue = {
			...
			["Color Hub 2"] = Color3.fromRGB(0, 120, 255),
			["Color Stroke"] = Color3.fromRGB(0, 90, 200),
...
local function AddHoverEffect(frame)
    ...
    gradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 0, 255)), -- ชมพู
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 255, 255))  -- ฟ้า
    }
...
local function OpenUI(ui)
    ui.Position = UDim2.new(0.5, 0, 0.5, 50)
    ui.Size = UDim2.new(0, 300, 0, 100)
    ui.Visible = true
...
local function CloseUI(ui)
    TweenService:Create(
        ...
        {Size = UDim2.new(0, 300, 0, 100), Position = UDim2.new(0.5, 0, 0.5, 50)}
    ):Play()
...
local function ApplyAnimationsToUI(screenGui)
...
ApplyAnimationsToUI(game:GetService("CoreGui"):FindFirstChild("redz Library V5"))
        ...
        for _, v in pairs(redzlib.Instances) do
            if v.Type == "Theme" then
                local tween = TweenService:Create(
                    ...
                    { [GetColor(v.Instance)] = Color3.fromHSV(tick() % 5 / 5, 1, 1) }
                )
                tween:Play()
            end
        end
